...
public class UTMRef extends CoordinateSystem {
    ...
    public UTMRef(int lngZone, char latZone, double easting, double northing) throws NotDefinedOnUTMGridException {
        ...
        if (lngZone < 1 || lngZone > 60) {
            throw new NotDefinedOnUTMGridException("Longitude zone (" + lngZone + ") is not defined on the UTM grid.");
        }

        if (latZone < 'C' || latZone > 'X') {
            throw new NotDefinedOnUTMGridException("Latitude zone (" + latZone + ") is not defined on the UTM grid.");
        }

        if (easting < 0.0 || easting > 1000000.0) {
            throw new NotDefinedOnUTMGridException("Easting (" + easting + ") is not defined on the UTM grid.");
        }

        if (northing < 0.0 || northing > 10000000.0) {
            throw new NotDefinedOnUTMGridException("Northing (" + northing + ") is not defined on the UTM grid.");
        }
