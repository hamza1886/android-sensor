...
public class GpsSnrView extends View {
    ...
    private void doInit(Context context) {
		...
		*/
         ...
         */
        preferredHeight = (int) ((2 * labelPaint.getTextSize() + context.getResources().getDimensionPixelSize(R.dimen.abc_text_size_medium_material)) * textHeight / labelPaint.getTextSize());
    }
    ...
    private void drawGrid(Canvas canvas) {
        ...
        // left boundary
        canvas.drawLine((float) gridStrokeWidth / 2, 0, (float) gridStrokeWidth / 2, h - textHeight, gridPaintStrong);
        ...
        for (int nmeaID = 1; nmeaID < MAX_NMEA_ID; nmeaID++) {
            ...
            if (pos > 0) {
                float x = (float) gridStrokeWidth / 2 + pos * (w - gridStrokeWidth) / numBars;
        ...
        // right boundary
        canvas.drawLine(w - (float) gridStrokeWidth / 2, h - textHeight, w - (float) gridStrokeWidth / 2, 0, gridPaintStrong);

        // bottom line
        canvas.drawLine(0, h - textHeight - (float) gridStrokeWidth / 2, w, h - textHeight - (float) gridStrokeWidth / 2, gridPaintStrong);
    }
    ...
    protected void initializeGrid() {
        ...
        if (mSats != null)
            for (GpsSatellite sat : mSats) {
                int prn = sat.getPrn();
                if (prn < 1) {
                    Log.wtf(TAG, String.format(Locale.getDefault(), "Got satellite with invalid NMEA ID %d", prn));
                } else if (prn <= 32) {
                    draw_1_32 = true;
                } else if (prn <= 54) {
                    draw_33_54 = true;
                    ...
                    Log.w(TAG, String.format(Locale.getDefault(), "Got satellite with NMEA ID %d (from the huge unassigned 97-192 range)", prn));
                } else if (prn <= 195) {
                    draw_193_195 = true;
                } else if (prn <= 200) {
                    ...
                    Log.w(TAG, String.format(Locale.getDefault(), "Got satellite with NMEA ID %d, possibly unsupported system", prn));
                }
            }
        /*
