...
import com.drunkdeveloper.location.sensor.radio.network.wifi.status.BuildConfig;
...
import com.drunkdeveloper.location.sensor.radio.network.wifi.status.widgets.GpsSnrView;
import com.drunkdeveloper.location.sensor.radio.network.wifi.status.widgets.GpsStatusView;
...
public class GpsSectionFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        AdRequest mAdRequest = BuildConfig.DEBUG ? new AdRequest.Builder().addTestDevice("1B37B6640CB64A6EE9227A806DD4470A").build() : new AdRequest.Builder().build();
    ...
    public void onLocationChanged(Location location) {
        if (location.hasAccuracy()) {
            ...
            gpsAccuracy.setText(String.format(Locale.getDefault(), "%.0f", getAcc));
        ...
        if (mainActivity.prefCoord == Const.KEY_PREF_COORD_DECIMAL) {
            ...
            gpsLat.setText(String.format(Locale.getDefault(), "%.5f%s", location.getLatitude(), getString(R.string.unit_degree)));
            gpsLon.setText(String.format(Locale.getDefault(), "%.5f%s", location.getLongitude(), getString(R.string.unit_degree)));
            ...
            gpsLat.setText(String.format(Locale.getDefault(), "%.0f%s %.3f'", deg, getString(R.string.unit_degree), min + /*rounding*/ 0.0005));
            dec = location.getLongitude();
            deg = (int) dec;
            min = Math.abs(60.0 * (dec - deg));
            gpsLon.setText(String.format(Locale.getDefault(), "%.0f%s %.3f'", deg, getString(R.string.unit_degree), min + /*rounding*/ 0.0005));
            ...
            gpsLat.setText(String.format(Locale.getDefault(), "%.0f%s %.0f' %.1f\"", deg, getString(R.string.unit_degree), min, sec + /*rounding*/ 0.05));
            dec = location.getLongitude();
            deg = (int) dec;
            tmp = 60.0 * (dec - deg);
            min = (int) Math.abs(tmp);
            sec = Math.abs(60.0 * (tmp - min));
            gpsLon.setText(String.format(Locale.getDefault(), "%.0f%s %.0f' %.1f\"", deg, getString(R.string.unit_degree), min, sec + /*rounding*/ 0.05));
        ...
        if (location.hasAltitude()) {
            ...
            gpsAlt.setText(String.format(Locale.getDefault(), "%.0f", getAltitude));
            gpsAltUnit.setText(getString(((mainActivity.prefUnitType) ? R.string.unit_meter : R.string.unit_feet)));
            orDeclination.setText(String.format(Locale.getDefault(), "%.0f%s", new GeomagneticField(
        ...
        if (location.hasBearing()) {
            gpsBearing.setText(String.format(Locale.getDefault(), "%.0f%s", location.getBearing(), getString(R.string.unit_degree)));
        ...
        if (location.hasSpeed()) {
            ...
            gpsSpeed.setText(String.format(Locale.getDefault(), "%.0f", getSpeed));
