...
import com.drunkdeveloper.location.sensor.radio.network.wifi.status.GpsEventReceiver;
...
import com.drunkdeveloper.location.sensor.radio.network.wifi.status.data.CellTowerList;
...
public class MainActivity extends AppCompatActivity implements GpsStatus.Listener, LocationListener, OnSharedPreferenceChangeListener, SensorEventListener {
     ...
     * <p>
     * This is chosen to meet the mission of app, which is to let the human know the values reported
     ...
     *
     * @param lists An array of {@link com.drunkdeveloper.location.sensor.radio.network.wifi.status.data.CellTowerList}
     *              instances
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Thread.setDefaultUncaughtExceptionHandler(new UncaughtExceptionHandler() {
            public void uncaughtException(Thread t, Throwable e) {
                ...
                String fileName = String.format(Locale.getDefault(), "satstat-%s.log", fmt.format(new Date(System.currentTimeMillis())));
        ...
        isWideScreen = (config.orientation == Configuration.ORIENTATION_LANDSCAPE && (rot == Surface.ROTATION_0 || rot == Surface.ROTATION_180) ||
                config.orientation == Configuration.ORIENTATION_PORTRAIT && (rot == Surface.ROTATION_90 || rot == Surface.ROTATION_270));
    ...
    protected void registerLocationProviders() {
        Set<String> providers = new HashSet<String>(mSharedPreferences.getStringSet(Const.KEY_PREF_LOC_PROV, new HashSet<String>(Arrays.asList(LocationManager.GPS_PROVIDER, LocationManager.NETWORK_PROVIDER))));
        locationManager.removeUpdates(this);
