...
package com.drunkdeveloper.location.sensor.radio.network.wifi.status;
...
import com.drunkdeveloper.location.sensor.radio.network.wifi.status.ui.MainActivity;
import com.drunkdeveloper.location.sensor.radio.network.wifi.status.utils.PermissionHelper;
import com.hzi.UTM;
...
public class PasvLocListenerService extends Service implements GpsStatus.Listener, LocationListener, OnSharedPreferenceChangeListener, OnRequestPermissionsResultCallback {
    ...
    public void onLocationChanged(Location location) {
        ...
        if (mNotifyFix && (mStatus != GPS_INACTIVE)) {
            ...
            if (prefCoord == Const.KEY_PREF_COORD_DECIMAL) {
                title = String.format(Locale.getDefault(),"%.5f%s%s %.5f%s%s",
                ...
                title = String.format(Locale.getDefault(),"%.0f%s %.3f' %s %.0f%s %.3f' %s",
                ...
                title = String.format(Locale.getDefault(), "%.0f%s %.0f' %.1f\" %s %.0f%s %.0f' %.1f\" %s",
            ...
            if (location.hasAccuracy()) {
                text = text + (text.equals("") ? "" : ", ") + String.format(Locale.getDefault(), "\u03b5 = %.0f%s",
                        (location.getAccuracy() * (prefUnitType ? 1 : 3.28084)),
                        getString(((prefUnitType) ? R.string.unit_meter : R.string.unit_feet)));
            }
            text = text + (text.equals("") ? "" : ", ") + String.format(Locale.getDefault(), "%d/%d",
                    satsUsed,
                    satsInView);
            text = text + (text.equals("") ? "" : ",\n") + String.format(Locale.getDefault(), "TTFF %d s",
    ...
    public void onSharedPreferenceChanged(SharedPreferences sharedPreferences, String key) {
        switch (key) {
            case Const.KEY_PREF_NOTIFY_FIX:
            case Const.KEY_PREF_NOTIFY_SEARCH:
