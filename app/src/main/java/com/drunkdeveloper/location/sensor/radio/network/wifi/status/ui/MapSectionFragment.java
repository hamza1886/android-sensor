...
public class MapSectionFragment extends Fragment {
    ...
    protected void applyLocationProviderStyle(Context context, String provider, String styleName) {
        ...
        if (circle != null) {
            circle.getPaintFill().setColor(style.getColor(Const.STYLE_FILL, getResources().getColor(R.color.circle_gray_fill)));
            circle.getPaintStroke().setColor(style.getColor(Const.STYLE_STROKE, getResources().getColor(R.color.circle_gray_stroke)));
    ...
    private void createLayers(boolean createOverlays) {
        ...
        if (mainActivity.prefMapOffline && (ContextCompat.checkSelfPermission(mainActivity, Manifest.permission.WRITE_EXTERNAL_STORAGE) == PackageManager.PERMISSION_GRANTED)) {
            ...
            Log.i(TAG, String.format(Locale.getDefault(), "Looking for maps in: %s", mapDir.getName()));
                ...
                for (File file : mapDir.listFiles())
                    if (file.isFile())
                        try {
                            ...
                            Log.i(TAG, String.format(Locale.getDefault(), "Added map file: %s", file.getName()));
                        } catch (MapFileException e) {
                            // not a map file, skip
                            Log.w(TAG, String.format(Locale.getDefault(), "Could not add map file: %s", file.getName()));
                        }
        ...
        if (createOverlays)
            onLocationProvidersChanged(mainActivity.mSharedPreferences.getStringSet(Const.KEY_PREF_LOC_PROV, new HashSet<String>(Arrays.asList(LocationManager.GPS_PROVIDER, LocationManager.NETWORK_PROVIDER))));
    ...
    public void onLocationProvidersChanged(Set<String> providers) {
        ...
        for (String pr : providers) {
            ...
            fill.setColor(style.getColor(Const.STYLE_FILL, getResources().getColor(R.color.circle_gray_fill)));
            fill.setStyle(Style.FILL);
            Paint stroke = AndroidGraphicFactory.INSTANCE.createPaint();
            stroke.setColor(style.getColor(Const.STYLE_STROKE, getResources().getColor(R.color.circle_gray_stroke)));
